...
def generated_generator(args, device, adj, features, labels, feature_normalized, adj_normalized, idx_train):
    x_list, c_list = [], []
    for i in trange(adj.shape[0]): # trange相当于tqdm和range的结合，可以显示进度条
        x = features[adj[i].nonzero()[1]]
        c = np.tile(features[i], (x.shape[0], 1)) # np.tile(A, reps)对输入数组，元组或列表进行reps次复制，并返回新的数组
        x_list.append(x)
        c_list.append(c)
    ...
    model_optimizer = optim.Adam(model.aprameters(), lr=lr, weight_decay=weight_decay)
    ...
    # pretain
    cvae = VAE(encoder_layer_size=[features.shape[1], 256],
